...
```{r, message=FALSE}
close_pair_dat <- bind_rows(bind_cols(snvthresh = '< 7 SNVs', pair_types %>% filter(pairwise_dist < 7) %>%
                     mutate(loc1=factor(loc1), pair_type=factor(pair_type)) %>%
                     group_by(loc1, pair_type) %>% summarize(n = n())
                     %>% tidyr::complete(loc1, pair_type)),
                    bind_cols(snvthresh = '< 11 SNVs', pair_types %>% filter(pairwise_dist < 11) %>%
                     mutate(loc1=factor(loc1), pair_type=factor(pair_type)) %>%
                     group_by(loc1, pair_type) %>% summarize(n = n())
                     %>% tidyr::complete(loc1, pair_type)))
...
snv_summary <- summarize_pairs(pair_types,summary_fns = c('min'), threshs = c(7, 11)) %>% filter(loc1 != loc2)
snv_summary
```
...
snv_and_metadat <- left_join(pair_types, metadata, by = c('sample1' = 'isolate_id')) %>% 
