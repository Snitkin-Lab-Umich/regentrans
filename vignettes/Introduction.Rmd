...
Sometimes it's useful to make plots faceted or subsetted based on various information from metadata such as collection date, sequence type, etc. To do this, you can merge the `snv_dists` object with the `metadata` object as follows (modifying the code as fit for your dataset):

```{r}
# merge snv_dists and metadata, and find time difference
snv_and_metadat <- left_join(snv_dists, metadata, by = c('sample1' = 'isolate_id')) %>% 
  rename(patient_id_1 = patient_id, collection_date_1 = collection_date) %>% 
  left_join(metadata, by = c('sample2' = 'isolate_id')) 
