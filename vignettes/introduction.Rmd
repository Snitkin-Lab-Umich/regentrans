---
title: "Introduction to `regentrans`"
...
We recommend using [IQ-TREE](http://www.iqtree.org/) to generate a maximum likelihood phylogenetic tree for your dataset. However, there are also ways to make simple phylogenies in R using a variant alignment. 
...
We can use `get_snv_dists()` to get the pair types for each isolate pair (intra- or inter-facility) and `get_frac_intra()` to calculate the fraction of intra-facility pairs for different SNV distances:
...
The output of `get_snv_dists()` includes information about each isolate pair including the pairwise distance, location of each isolate in the pair, and the type of pair (intra- or inter-facility): 
...
On of these (11) matches with Method 1. 
We also use 7 as a more conservative distance threshold for our sensitivity analysis.
...
To explore this question we can leverage our whole-genome sequencing data in the form of the phylogenetic tree using the `get_clusters()` function and the SNV distance matrix using the `get_snv_dists()`. 
...
```{r}
ggplot(data = pure_subtree_info, aes(x = loc_id, y = subtr_size, color = loc_id)) + 
...
*Note:* If you do not have a patient transfer network, you could use geographic distances between facilities as a proxy for patient transfer because they are often correlated. 
...
First, we can summarize the pairwise SNV distance information using the `summarize_pairs()` function:
...
The function allows you to input a vector of whatever summary functions you'd like, as well as pairwise SNV distance thresholds to compute the number of closely related pairs for each facility pair under that threshold. 
...
```{r}
pair_info %>% 
  ggplot(aes(x=sum_pt_trans_metric,y=under_7)) +
  geom_point() + geom_smooth(method='lm') + scale_x_log10() +
...
To evaluate whether there are any observable geographic trends in prevalence or transmission, we will need geographic information (latitude and longitude) for each facility. Here we've de-identified the facility geographies as an example:

```{r}
facil_geo
```

This data frame contains the facility ID, longitude, and latitude. 

We can use this to plot the facilities on a map and connect them by closely related pairs:
