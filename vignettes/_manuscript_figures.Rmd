...
pair_types <- get_pair_types(dists = dists, locs = locs) #%>% subset_pairs()
pat_pair_types <- get_pair_types(dists = pat_dists, locs = locs) #%>% subset_pairs()
...
pat_frac_intra <- pat_pair_types_new %>% select(-orig_dist_bin, -orig_dist) %>% 
...
f2 <- plot_grid(f2a, f2b, ncol = 1, align = 'hv', labels = 'AUTO')
...
f4a <- pw_snv_plot + annotation_custom(ggplotGrob(pw_snv_plot_zoom), 
                                      xmin = 250, xmax = 450, ymin = 200, ymax = 2800)
...
pat_pw_snv_plot <- pat_pair_types %>% 
  ggplot(aes(x = pairwise_dist, fill = pair_type)) + 
  geom_histogram(position = 'identity', alpha = 0.4, bins = 30) +
  labs(x = "Patristic distance", y = "# pairs", fill = "") +
...
f4b <- pat_pw_snv_plot + annotation_custom(ggplotGrob(pat_pw_snv_plot_zoom), 
                                    xmin = 5e-5, xmax = 9e-5, ymin = 200, ymax = 2800)
...
flow_min_snv_plot <- pair_info %>%
  ...
  annotate('text', x = 0.004, y = 30, 
           label = paste(paste('Spearman rho =', round(flow_min_snv_spear$estimate, 2)),'\n',
                         paste('Spearman p =', signif(flow_min_snv_spear$p.value, 1)))) + ylim(c(0,50))

f7 <- plot_grid(flow_fsp_plot, flow_min_snv_plot, flow_snv_plot, ncol = 1, labels = 'AUTO')
ggsave('manuscript_figures/Fig7.png',f7, width = 6, height = 8)
f7
####__________####
...
time2 <- pair_types %>% 
  ...
  # xlim(0,20) +
  labs(x = "Pairwise SNV distance", y = "Fraction of pairs", fill = "") +
  facet_grid(~date1) + 
  theme(legend.justification = "top", 
        axis.line = element_line(colour = "black"),
        panel.border = element_blank())
...
geo_map <- facil_geo %>% ggplot(aes(x=long, y=lat)) +
  ...
  labs(size = '# samples', alpha = '# closely related\npairs (≤ 6 SNVs)')
