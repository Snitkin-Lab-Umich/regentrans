...
coldat <- bind_rows(bind_cols(snvthresh = '< 7 SNVs', pair_types %>% filter(Pairwise_Dists < 7) %>%
                     mutate(Loc1=factor(Loc1), Pair_Type=factor(Pair_Type)) %>%
                     group_by(Loc1, Pair_Type) %>% summarize(n = n())
                     %>% tidyr::complete(Loc1, Pair_Type)),
                    bind_cols(snvthresh = '< 11 SNVs', pair_types %>% filter(Pairwise_Dists < 11) %>%
                     mutate(Loc1=factor(Loc1), Pair_Type=factor(Pair_Type)) %>%
                     group_by(Loc1, Pair_Type) %>% summarize(n = n())
                     %>% tidyr::complete(Loc1, Pair_Type)))
...
snv_dist_summary <- pair_types %>% dplyr::group_by(Loc1, Loc2) %>% dplyr::summarize(n_closely_related_pairs = sum(Pairwise_Dists <= thresh)) %>% dplyr::filter(Loc1 != Loc2)

# USE MIN INSTEAD/IN ADDITION?
# snv_dist_summary <- pair_types %>% dplyr::group_by(Loc1, Loc2) %>% dplyr::summarize(n_closely_related_pairs = min(Pairwise_Dists)) %>% dplyr::filter(Loc1 != Loc2)
...
snv_geo <- pair_types %>% dplyr::group_by(Loc1, Loc2) %>% dplyr::summarize(n_closely_related_pairs = sum(Pairwise_Dists < 7)) %>% dplyr::filter(Loc1 != Loc2, n_closely_related_pairs > 0) %>%
