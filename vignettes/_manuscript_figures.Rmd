...
f2b <- pat_frac_intra %>% 
  ...
  mutate(under = ifelse(orig_dist_bin %in% levels(pat_frac_intra$orig_dist_bin)[1:4], 
                        "Branch length '≤'~2.56*'\u00D7'*10^6~mu/site/yr",'Over threshold'),
  under = factor(under, levels = c("Branch length '≤'~2.56*'\u00D7'*10^6~mu/site/yr",'Over threshold')),
  ...
  scale_fill_manual(values = c('black', 'lightgrey'), 
                    labels =  c(expression(paste("≤ 2.56", "\u00D7", 10^6, mu, "/site/yr")),'Over threshold       ')) + 
...
# ggsave('manuscript_figures/Fig2.tiff', f2, width = 7, height = 5, dpi=1200)
tiff('manuscript_figures/Fig2.tiff', width = 7, height = 5, units='in', res=600)
f2
dev.off()
f2
```
...
pw_snv_plot <- pair_types %>% 
  ...
  # scale_fill_manual(values = c('Inter-facility pair' = 'grey','Intra-facility pair' = 'black')) +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(x = "Pairwise SNV distance", y = "Number of pairs", fill = "") +
...
pw_snv_plot_zoom <- pair_types %>% 
  ...
  # scale_fill_manual(values = c('Inter-facility pair' = 'grey','Intra-facility pair' = 'black')) +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(x = "Pairwise SNV\ndistance", y = "Number of pairs", fill = "") +
...
pat_pw_snv_plot <- pat_pair_types %>% 
  ...
  # scale_fill_manual(values = c('Inter-facility pair' = 'grey','Intra-facility pair' = 'black')) +
  guides(fill = guide_legend(reverse = TRUE)) +
  labs(x = "Pairwise patristic distance", y = "Number of pairs", fill = "") +
...
# ggsave('manuscript_figures/Fig3.tiff', f3, height = 7, width = 6, dpi=1200)

f3 <- plot_grid(f3a, f3b, ncol = 1, labels = 'AUTO')
ggsave('manuscript_figures/Fig3.png', f3, height = 7, width = 6)
tiff('manuscript_figures/Fig3.tiff', width = 6, height = 7, units='in', res=600)
f3
dev.off()
f3
...
tiff('manuscript_figures/Fig4.tiff', width = 8, height = 8, units='in', res=600)
# ggsave('manuscript_figures/Fig4.tiff', f4, width = 8, height = 8, dpi=1200)
f4
dev.off()
f4
```
...
phm <- pheatmap(fsp, border_color = 'white',
         legend_breaks = c(0, 0.2, 0.4, 0.6, 0.8, max(fsp)), 
         legend_labels = c(0, 0.2, 0.4, 0.6, 0.8, 'Fsp'))

# tiff('manuscript_figures/Fig5.tiff', width = 7, height = 6, units = 'in', res=1200)
# grid::grid.newpage()
...
pat_summary_2 <- pat_summary
colnames(pat_summary_2) <- c("loc2", "loc1", "dist_min", "leq_2.56e-06")
pat_summary_2 <- pat_summary_2 %>% relocate(loc1, .before = loc2) %>% filter(loc1 != loc2)
pat_summary_comb <- rbind(pat_summary, pat_summary_2)
...
facil_summary <- left_join(snv_summary_comb, pat_summary_comb %>% select(-dist_min)) %>% 
  ...
  # mutate(thresh = paste0(gsub('leq_','≤~', thresh), '~SNVs'),
         # thresh = gsub('2.56e-06~SNVs', '2.56%*%10^-06\nmutations/site/year', thresh)) 

# get locations to keep (at least 1 closely related pair)
keep_locs <- facil_summary %>% group_by(loc1) %>% summarize(sum_all = sum(n_pairs, na.rm = TRUE)) %>% filter(sum_all != 0) %>% select(loc1) %>% deframe()
```

```{r}
# plot
f6a <- facil_summary %>% filter(loc1 %in% keep_locs) %>% 
  ...
  geom_col(position = 'dodge') +
       # scale_fill_manual(values=c("cadetblue", "salmon")) + 
         labs(y = "Number of closely related isolate pairs", x = "Facility", fill = '', color = '') +
  scale_fill_manual(values = c('Inter-facility pair' = 'grey','Intra-facility pair' = 'black')) +
  guides(fill = guide_legend(reverse = TRUE)) +
  facet_grid(factor(thresh, levels = c('leq_6', 'leq_10', 'leq_2.56e-06'),
                    labels = c("'≤'~6~SNVs", "'≤'~10~SNVs","'≤'~2.56*'\u00D7'*10^6~mu/site/yr"))~., 
                               labeller = label_parsed) +
  theme(legend.position = 'bottom', strip.text = element_text(size = 10))

g <- pair_types %>% filter(pairwise_dist <= 6) %>% select(pt1, pt2) %>% graph_from_data_frame() 
E(g)$pair_type <- pair_types %>% filter(pairwise_dist <= 6) %>% pull(pair_type)
pt_locs <- metadata %>% select(patient_id, facility) %>% deframe()
pt_locs <- pt_locs[names(V(g))]
V(g)$loc <- pt_locs
  
set.seed(0)
f6b <- ggraph(g, layout = 'kk') +
  geom_edge_link(aes(col = pair_type), alpha = 0.5) + 
    geom_node_point(aes(col = loc)) +
  scale_edge_color_manual(values = c('Inter-facility pair' = 'grey','Intra-facility pair' = 'black')) +
  scale_color_manual(values = cols) +
  guides(color = guide_legend(ncol = 2), edge_color = guide_legend(reverse = TRUE)) +
  labs(edge_color = 'Pair type\n(≤ 6 SNVs)', color = 'Patient\'s\nfacility') +
  theme_void() + theme(text=element_text(size=15))

# plot_grid(f6b, NULL, ncol = 1, rel_heights = c(0.9,0.1))

f6 <- plot_grid(f6a, plot_grid(f6b, NULL, ncol = 1, rel_heights = c(0.9,0.1)), rel_widths = c(0.4,0.6), labels = 'AUTO')
ggsave('manuscript_figures/Fig6.png',f6, width = 10, height = 5.5)
# ggsave('manuscript_figures/Fig6.tiff', f6, width = 10, height = 5.5, dpi=1200)
tiff('manuscript_figures/Fig6.tiff', width = 10, height = 5.5, units='in', res=600)
f6
dev.off()
f6
```
...
# ggsave('manuscript_figures/Fig7.tiff', f7, width = 7, height = 5, dpi=1200)
tiff('manuscript_figures/Fig7.tiff', width = 7, height = 5, units='in', res=600)
f7
dev.off()
f7
```
```{r}
pt_flow_long <- make_long_form(pt_flow, col_names = c('loc1','loc2','pt_flow')) 

facil_pairs <- lapply(1:nrow(pt_flow_long), function(x)
      sort(c(as.character(pt_flow_long$loc1[x]), as.character(pt_flow_long$loc2[x]))))

pt_flow_long$loc1 <- sapply(facil_pairs, function(x) x[1])
pt_flow_long$loc2 <- sapply(facil_pairs, function(x) x[2])

pt_flow_long_mean <- pt_flow_long %>% arrange(loc1, loc2) %>% group_by(loc1, loc2) %>% 
  summarize(mean_pt_flow = mean(pt_flow))
...
```{r}
flow_fsp_spear <- cor.test(pair_info$mean_pt_flow[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$fsp)],
                                 pair_info$fsp[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$fsp)], 
                                 method = 'spearman')

flow_fsp_plot <- pair_info %>% 
  filter(!is.na(mean_pt_flow)) %>% 
  ggplot(aes(x=mean_pt_flow,y=fsp)) +
  geom_point() + geom_smooth(method='lm', col = 'grey50') + scale_x_log10(label=fancy_scientific) +
  labs(x = 'Mean patient flow', y = '\n\nFsp') + 
  ...
  theme(plot.margin = margin(t = 0, r = 0.5, b = 0.5, l = 0, unit = "cm"))

flow_snv_spear <- cor.test(pair_info$mean_pt_flow[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$leq_10)],
                                 pair_info$leq_10[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$leq_10)], 
                                 method = 'spearman')

flow_snv_plot <- pair_info %>% 
  filter(!is.na(mean_pt_flow)) %>% 
  ggplot(aes(x=mean_pt_flow,y=leq_10)) +
  geom_point() + geom_smooth(method='lm', col = 'grey50') + scale_x_log10(label=fancy_scientific) +
  labs(x = 'Mean patient flow', y = 'Number of\nclosely related\npairs (≤ 10 SNVs)') + 
  annotate('text', x = 0.004, y = 8, 
           label = paste(paste('Spearman rho =', round(flow_snv_spear$estimate, 2)),'\n',
                         paste('Spearman p =', signif(flow_snv_spear$p.value, 1)))) +
  theme(plot.margin = margin(t = 0, r = 0.5, b = 0.5, l = 0, unit = "cm"))

flow_min_snv_spear <- cor.test(pair_info$mean_pt_flow[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$dist_min)],
                                 pair_info$dist_min[!is.na(pair_info$mean_pt_flow) & !is.na(pair_info$dist_min)], 
                                 method = 'spearman')

flow_min_snv_plot <- pair_info %>%
  filter(!is.na(mean_pt_flow)) %>% 
  ggplot(aes(x=mean_pt_flow,y=dist_min)) +
  geom_point() + geom_smooth(method='lm', col = 'grey50') + scale_x_log10(label=fancy_scientific) +
  labs(x = 'Mean patient flow', y = '\nMinimum pairwise \nSNV distance') +
  annotate('text', x = 0.004, y = 30, 
           label = paste(paste('Spearman rho =', round(flow_min_snv_spear$estimate, 2)),'\n',
                         paste('Spearman p =', signif(flow_min_snv_spear$p.value, 1)))) +
  theme(plot.margin = margin(t = 0, r = 0.5, b = 0.5, l = 0, unit = "cm"))

f8 <- plot_grid(flow_fsp_plot, flow_min_snv_plot, flow_snv_plot, ncol = 1, labels = 'AUTO')
ggsave('manuscript_figures/Fig8.png',f8, width = 6, height = 8)
# ggsave('manuscript_figures/Fig8.tiff', f8, width = 6, height = 6, dpi=1200)
tiff('manuscript_figures/Fig8.tiff', width = 6, height = 8, units='in', res=600)
f8
dev.off()
f8
```
...
geo_map <- facil_geo %>% ggplot(aes(x=long, y=lat)) +
  ...
  labs(size = 'Number of\nsamples', fill = 'Number of\nsamples', col = 'Number of\nclosely related\npairs (≤ 6 SNVs)')
...
geo_fsp <- geo_dat %>% 
  ...
  geom_point() + geom_smooth(method = 'lm', col = 'grey50') +
...
geo_min_snv <- geo_dat %>% 
  filter(!is.na(geo_dist)) %>% 
  ggplot(aes(x = geo_dist, y = dist_min)) + geom_point() + geom_smooth(method = 'lm', col = 'grey50') +
...
geo_snv <- geo_dat %>% 
  filter(!is.na(geo_dist)) %>% 
  ggplot(aes(x = geo_dist, y = leq_10)) + geom_point() + geom_smooth(method = 'lm', col = 'grey50') +
  labs(x = 'Physical distance between facilities', y = 'Number of closely related pairs\n(≤ 10 SNVs)') + 
...
tiff('manuscript_figures/Fig9.tiff', width = 12, height = 12, units='in', res=400)
# ggsave('manuscript_figures/Fig9.tiff', f9, width = 12, height = 12, dpi=600)
f9
dev.off()
f9
```
