...
test_that("get_snv_dists works", {
  ...
  expect_true(all(filter(test_pure_subtree_info, !is.na(index))$isolate_name == " "))
  #make sure if there is.na(index) size == 1 and isolate name != " "
  expect_true(all(filter(test_pure_subtree_info, is.na(index))$isolate_name != " ") & all(filter(test_pure_subtree_info, is.na(index))$subtr_size == 1))
