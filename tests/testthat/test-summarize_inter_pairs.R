# tests for summarize_inter_pairs

locs <- metadata %>% dplyr::select(isolate_id, facility) %>% tibble::deframe()
test_locs <- locs[1:3]
test_dists <- dists[names(test_locs), names(test_locs)]
test_snv_dists <- get_snv_dists(dists = test_dists, locs = test_locs)
test_inter_pair_summary <- summarize_inter_pairs(test_snv_dists, summary_fns = c("min"), threshs = c(20, 50))
test_inter_pair_summary_2 <- summarize_inter_pairs(test_snv_dists, summary_fns = c("min"), threshs = NULL)
test_inter_pair_summary_3 <- summarize_inter_pairs(test_snv_dists, summary_fns = NULL, threshs = c(20, 50))
...
test_that("summarize_inter_pairs works", {
  # everything
...
test_that("merge_inter_summaries works", {
  expect_equal(merged_summaries,
               structure(list(Loc1 = c("A", "A", "B", "A"),
                              ...
                              Fsp = c(NA, 0.98922544689928, NA, 0.728466871240393),
  ...
  expect_equal(merged_summaries_3,
               structure(list(Loc1 = c("A", "A"), Loc2 = c("C", "D"),
                              Fsp = c(0.98922544689928, 0.728466871240393),
