...
Permutation can be used as a method to check whether your data is enriched for transmission within/between certain facilities. For example, if we permute the locaiton labels on the `snv_dists` output before calculating the number of isolates below a certain threshold at each facility, we can interrogate whether the enrichments in our data are significant. Permutation can also be used to address significance of other regentrans methods. 
...
apply(permut, MARGIN = 1, FUN = function(row){
